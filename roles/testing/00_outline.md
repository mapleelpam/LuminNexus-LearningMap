...
> **📌 主題檔**（01-06 已成文）：
> 1. [01_product-understanding.md](./01_product-understanding.md) - 產品理解
> 2. [02_testing-basics.md](./02_testing-basics.md) - 測試基礎概念
> 3. [03_rest-api-basics.md](./03_rest-api-basics.md) - REST API 基礎
> 4. [04_mdfo-query-understanding.md](./04_mdfo-query-understanding.md) - MDFO 查詢理解
> 5. [05_test-execution-practice.md](./05_test-execution-practice.md) - 測試執行實務（Postman、F12、Excel）
...
- **功能測試** - 單一功能是否正常運作
- **回歸測試** - 改動後，舊功能是否仍正常
- **驗收測試** - 從使用者角度，整體流程是否走得通

#### 2.3 測試流程
- **1. 設計** - 理解需求，想清楚要驗證什麼
- **2. 執行** - 實際操作並觀察結果
- **3. 記錄** - 如實寫下實際結果
- **4. 回報** - 發現問題就講清楚（修復後回到執行重新驗證）
...
#### 5.2 瀏覽器開發者工具
- **F12 → Network** - 觀察前端實際送出的查詢與回應
- **用途** - 分辨「前端送錯查詢」還是「後端回錯答案」

#### 5.3 執行現成測試腳本
- **執行** - quick_test.py（位於產品 repo，執行即可、不需懂 Python）
- **判讀輸出** - 通過/失敗摘要、Expected vs Actual、記錄失敗案例

#### 5.4 前端 UI 測試（手動）
- **通用檢查法** - 先有預期，像使用者一樣操作、對照觀察
...
#### 6.1 資料驗證基礎
- **對照已知總數** - 部分不可能大於全部
- **量級判斷** - 看位數不看尾數，「合理」不是「精確」
- **必要欄位齊全** - 回應形狀與查詢結構對得上

#### 6.2 分類異常察覺
- **分工邊界** - 分類法（Taxonomy）的系統性驗證由資料團隊負責
- **察覺訊號** - 父子倒掛、雙胞胎分組（名稱不一致）
- **處理方式** - 記錄並回報，不需自行修正
...
- `tests/README.md` - 完整測試架構說明（參考）
- `tests/MULTI_DIMENSION_BUSINESS_TESTING.md` - 商業場景測試範例（參考）
...
### Q: 需要學 SQL 嗎？
A: 不需要。資料層的系統性驗證（例如掃描整棵分類樹）由資料團隊負責；測試角色透過查詢結果做合理性判斷即可（見 06 章）。
