...
facet、taxonomy、state 講的是「這東西是什麼」，instance 講的是「它在哪一層」——tag 兩者都不是。**tag 講的是「你選擇用什麼方式記錄它」**。

所以 `too_sweet` 本身不是 tag，它是「**被記成一個裸標籤、而不是某條軸的值**」的那個狀態。辨識方式很直接：看到 `tags: [too_sweet, effective, gummy]` 這種陣列，問一句「`too_sweet` 是味道那條軸的值？taxonomy 上的節點？還是一個會變的狀態？」——**檔案裡沒有任何東西回答得了**，因為歸屬沒被寫下來。三種不同身分的東西並排在同一個陣列裡，靠讀的人自己補上缺掉的語意。
