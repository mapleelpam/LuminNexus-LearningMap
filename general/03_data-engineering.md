...
audience:
  - all
summary: |
  資料格式（JSON/CSV/YAML）、設定檔階層的三種寫法與取捨、資料庫基礎、
  ETL 流程與資料充實技術。
---
...
- [1. 資料結構與格式](#1-資料結構與格式)
  ...
  - [1.5 設定檔裡的「階層」怎麼表達](#15-設定檔裡的階層怎麼表達)
- [2. 資料庫基礎](#2-資料庫基礎)
...
格式選定之後還有一個設計決策——**同一個格式裡，「A 底下有 B」要怎麼寫**。那是下一節。

---

### 1.5 設定檔裡的「階層」怎麼表達

設定檔常常要記一種東西：**A 底下有 B、B 底下有 C**。分類體系、選單、組織架構、目錄結構都是。

寫法有三種。**選錯的代價不會馬上出現**——檔案照樣能讀、程式照樣能跑；要等到一年後有人說「這個分類改個名字」或「把這一支搬到另一個大類底下」，才會發現改一個字要動全檔、而且舊資料接不回來。所以值得先知道差別。

（以下假設這個階層是**一棵樹**：每個東西只有一個上層。想同時掛在很多個上層的情況，見取捨表後面那一段。）

**寫法 A：巢狀（最直覺）**——階層就是檔案的縮排形狀
...
> 注意：`維生素C:` 後面**一定要有東西**（這裡是 `unit`）。冒號後面留空在 YAML 裡會被解析成 `null`，不是「空的分類」。

**寫法 B：扁平清單 ＋ 指向父節點**——階層寫在每筆紀錄的一個欄位裡
...
**寫法 C：物化路徑**——把從最上層到自己的整條路徑寫成一個字串（materialized path，資料庫領域的既有名稱）
...
| 面向 | A 巢狀 | B 扁平＋parent | C 物化路徑 |
|---|---|---|---|
| 人眼可讀 | ✅ 一目了然 | ❌ 要工具還原成樹 | ✅ 單行自解釋 |
| **改名字** | ❌ key 就是身分，改名＝改結構 | ✅ id 不動、只改 label | ❌ 所有後代路徑失效 |
| **搬動位置** | ❌ 整段搬家，版本控制的 diff 很難看 | ✅ 只改一行 | ⚠️ 後代路徑全要重寫 |
| 層數不固定 | ❌ 驗證規則要寫死每一層 | ✅ 天然無限深 | ✅ 天然無限深 |
| 防止繞圈<br/>（A 的父是 B、B 的父又是 A） | ✅ 語法上不可能 | ❌ **要自己檢查** | ✅ 語法上不可能 |
| 父節點指到不存在的東西 | ✅ 語法上不可能 | ❌ `parent: vitamn` 沒東西擋 | ✅ 語法上不可能 |
| 兄弟之間的顯示順序 | ✅ 清單順序即順序 | ❌ 要另外加 `order` 欄位 | ❌ 要另外加 `order` 欄位 |

**那「一個節點想掛在兩個父節點下」怎麼辦？**

寫法 B 技術上做得到（`parent` 改成陣列），但**先別急著這樣寫**。一棵樹的定義就是每個東西只掛一個位置；會想掛兩個，**通常代表你把兩條互相獨立的線壓成了一棵樹**——例如「劑型」和「適用對象」被硬塞進同一棵樹，於是「兒童軟糖」不知道該掛哪裡。正確處理是**拆成兩條獨立的軸**，各自記各自的值。（判斷方法見 [classification-terminology.md](./classification-terminology.md) 第 4 節。）

**實務建議**：

- **會長期演化、會改名、會搬動的**（分類體系、**受控詞彙**——事先定好一份合法值清單，只能從裡面挑）→ **用 B**，但要先看下一條
- **B 的前提是你有驗證機制。** 上表那三個 ❌（繞圈、指到不存在的東西、順序）在 A 是語法上不可能發生，在 B 全都要靠程式檢查——那代表一支腳本、掛進 CI、有人維護。業界確實有 B 的正統做法（W3C 的 SKOS 標準就是扁平＋`broader`），但 SKOS 是給機器讀的，而且配套有 qSKOS 這類驗證器。**沒有驗證機制就選 B，等於把不變式換成了口頭約定**
- **只是讀一讀、不太會改的**（一份匯出檔、一次性的設定）→ A 或 C 都好，怎麼順手怎麼來
- 不管用哪一種，**節點的識別碼不要用它的顯示名稱**。顯示名稱是會改的東西，拿它當身分，改名就等於把舊資料抹掉（識別碼什麼時候該換，見 [classification-terminology.md](./classification-terminology.md) §7）
...
| 1.1 | 2026-08-18 | 新增 §1.5 設定檔裡的階層怎麼表達（巢狀／扁平＋parent／物化路徑三種寫法與取捨） | Dustin |
| 1.2 | 2026-08-20 | 依 issue #12 修正 §1.5：寫法 A 範例改為合法且三層的 YAML；取捨表補「指到不存在的父節點」「兄弟順序」兩列，多父節點改列為警訊（該拆成兩條軸）；補 B 需要驗證機制（SKOS／qSKOS）的前提；解釋物化路徑、受控詞彙、繞圈；§1.4 小結加上通往 §1.5 的接續 | Dustin |
