...
> **怎麼讀**：先看下面「一句話總結」的 9 個詞，那是不懂就會看不下去的最小集合。其餘章節用到再回來查。
...
| **%DV / IU / CFU** | 標籤上的三種單位系統——重量、活性、當量，**混在一起不能比** |
...
> 📌 **Supplement Facts 不是 Nutrition Facts。** 食品用的是 Nutrition Facts（營養標示），補充劑用的是 Supplement Facts，兩者的法規依據與欄位都不同——最明顯的差別是 Supplement Facts 可以列沒有每日參考值的成分（草本萃取那類），Nutrition Facts 不行。**看到 Nutrition Facts 面板，那多半不是補充劑。**
...
上面這罐含 750 mg 的四種成分——但**沒有人知道其中的紅景天是 500 mg 還是 5 mg**。廠商的說法是保護配方機密，實務上它也讓「劑量不足」變得無法檢驗。
...
| **AI** | Adequate Intake | 證據不足以訂 RDA 時用的替代值 |
...
| **95% curcuminoids（標準化萃取）** | 有效成分佔比達 95% | 直接保證有效成分的比例 |

**兩種標法不能直接比較**——一個講來源有多濃，一個講成品裡有多少。這也是第 12 節第 ⑤ 問（「這個詞在這裡是哪個意思」）的典型例子。
...
**這一節有半數的概念都指向同一件事：bioavailability（生物利用率）**——吃下去的量裡，真正被身體吸收利用的比例。enabler 成分在提高它、螯合與微脂體這類技術在提高它、劑型選擇也在影響它。所以「含量高」不等於「吸收多」，兩罐標示劑量相同的產品，實際效果可以差很多。這個字是這個產業一半技術宣稱的共同目標。
...
| 詞 | 白話 |
|---|---|
| **channel type（通路角色）** | 這個賣家在商業上扮演什麼角色。常見四類：電商平台、品牌官網、零售商、法規資料庫 |
...
| **facet（分面）** | LanguaL 用來切分類的**軸**——同一個產品可以同時被好幾個 facet 描述，彼此不衝突。DSLD 的劑型就標在 **Facet A**。發音近 /ˈfæsɪt/（「花-sit」，不是「法-sei」）。⚠️ **本文只講 LanguaL 脈絡**；facet 當通用邏輯概念（正交性、為什麼不能塞成一棵樹）見 [classification-terminology.md](./classification-terminology.md) 第 2 節 |
