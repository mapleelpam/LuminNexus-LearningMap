---
title: "寫下來之前：facet、taxonomy、lifecycle、tag、instance"
type: guide
status: active
created: 2026-08-18
updated: 2026-08-21
...
summary: |
  ...
  五個名字（facet / taxonomy / lifecycle / tag / instance）各自回答什麼、
  看起來像樹的東西要過哪兩關、三份檔案為什麼要分開——
  給的是看得出對不對的判準，不是可以抄的成品。
  末段附五次實測：這五次裡模型都沒有把正交的面壓成樹，但它每次會生得不一樣。
---

# 寫下來之前：facet、taxonomy、lifecycle、tag、instance
...
這篇講**檔案已經在你面前的那一刻**——多半是 Claude 生的，你要決定收不收。
...
```text
階層由「檔案的形狀」承載        │  階層由「紀錄上的一個欄位」承載
                                │
- id: vitamin                   │  - id: vitamin
  label: 維生素                  │    label: 維生素
  children:                     │    parent: null
    - id: vitamin_c             │  - id: vitamin_c
      label: 維生素 C            │    label: 維生素 C
                                │    parent: vitamin
```

**這兩份檔案表達的是同一件事**——同樣的 id、同樣的 label，差別只在階層寫在哪裡。

拿到其中一份，你要認得出它是哪一種，因為兩種寫法各自**免費送你一些保證**，也各自把一些保證變成**你要自己做的事**：
...
- **「想把一個節點掛在兩個上層」是警訊，不是需求。** 欄位那條技術上做得到（`parent` 改成陣列），但一棵樹的定義就是每個東西只掛一個位置。會想掛兩個，通常代表**兩條互相獨立的軸被壓成了一棵樹**——例如「劑型」和「適用對象」被塞進同一棵樹，於是「兒童軟糖」不知道該掛哪。正確處理是拆成兩條軸，這正是第 3 節第一關在講的事

最後兩件事。

**以上都預設這個階層是一棵樹**（每個東西只有一個上層）。如果資料本身是一張網——一個東西連到很多個、甚至互相連——上面這些寫法都寫不出來，它在檔案裡會長成 `related: [...]` 這類欄位，那已經不是階層了。

**還有，這一節的坑不是所有格式都有。** 本文一律拿 YAML 舉例，因為那是團隊實際在用的格式；**三層分離對 JSON 一樣成立**，但「縮排被讀成階層」這個誤讀**只有用縮排表達包含關係的格式才會發生**——JSON 用括號就沒有這個問題。
...
|---|---|---|
| ① | 這在**哪一層**？ | 定義層 ／ 實例層 |
| ② | 若在定義層，它**扮演什麼**？ | 一條軸 ／ 軸底下怎麼分類 ／ 一個會走完的流程 |
...
| **lifecycle（生命週期）** | ②：會走完的流程 | **它會變，而且變了之後要留下歷史** |
...
### lifecycle 不是 facet value

最容易混的一組。判準是**「它會變嗎，變了之後舊值還算數嗎」**。

> 這件事在很多檔案裡的欄位名就叫 `status:`，概念上一般也叫 state。**本文用 `lifecycle` 稱呼它**，因為 repo 裡 [compute-state-context](./compute-state-context.md) 已經用 state 指另一件事——「呼叫與呼叫之間必須存活下來的資訊」。同一個字兩個意思，改個名字比每次提醒省事。
...
> **facet value 描述「這個東西是什麼」；lifecycle 描述「這筆資料在流程中走到哪」。**

**定義層的東西也有生命週期。** 一個分類節點會退役：`status: active` → `status: retired`——節點**還在**，只是不再建議使用，舊資料還掛在它上面，而且必須查得到「它什麼時候退役、改用誰」。
...
facet、taxonomy、lifecycle 講的是「這東西是什麼」，instance 講的是「它在哪一層」——tag 兩者都不是。**tag 講的是「你選擇用什麼方式記錄它」**。

所以 `too_sweet` 本身不是 tag，它是「**被記成一個裸標籤、而不是某條軸的值**」的那個狀態。辨識方式很直接：看到 `tags: [too_sweet, effective, gummy]` 這種陣列，問一句「`too_sweet` 是味道那條軸的值？taxonomy 上的節點？還是一個會變的狀態？」——**檔案裡沒有任何東西回答得了**，因為歸屬沒被寫下來。三種不同性質的東西並排在同一個陣列裡，靠讀的人自己補上缺掉的語意。

**在探索階段，這是合理的**——你還不知道有幾條軸，先把觀察到的東西記下來。問題出在它留太久：**一旦有人開始拿 tag 做統計，那個「還沒想清楚」就變成了地基**（代價有多具體，見第 5 節）。

**但別把所有裸標籤都當成錯誤。** 讓使用者自己貼、分類由下而上長出來，這種做法有正式名稱（**folksonomy／大眾分類**），而且至少三種情況 tag 是終點不是暫存：

- **那條軸本質上就是開放的**——使用情境、品牌聯想，你永遠列不完
- **刻意跨軸的流程標記**——`待複查`、`拿來訓練用`，它本來就不屬於任何一條描述性的軸
- **長尾的維護成本大於收益**——為了三筆資料開一條新軸並不划算

判準還是同一句：**有沒有人要拿它做統計。** 要，就得有歸屬；不要，裸標籤沒有問題。
...
taste:
  ...
  - balanced
  - not_sweet
selected_taste: too_sweet
```

這份 YAML 同時塞了**這一則評論的判讀結果**、**味道這條軸的定義**、**味道底下所有可能的值**。於是冒出 `selected_taste` 這種欄位——因為 `taste:` 那一坨已經分不出哪個是選項、哪個是選中的，只好再開一個名字很像的欄位裝答案。

**這是最好認的指紋**：看到 `selected_*`、`chosen_*`、`current_*` 這類欄位，回頭檢查它上面那個欄位到底是定義還是資料。
...
**「畫成樹」這個動作太順手了。** 一份真的檔案裡，最常見的兩種多做的層是：

- **它其實是幾條獨立的軸**——被疊成了一棵樹
- **它真的是一棵樹，但沒有人需要那一層**——多一層只是多一層要維護的東西
...
你想分析五件事：**味道、功效、劑型、副作用、價格**。這五個彼此正交——知道它太甜，推不出它有沒有效；知道它是軟糖，推不出它貴不貴。**五個 facet，沒有問題。**
...
> **這個例子不是錯的，它是真實世界的典型長相**：一份看起來像 taxonomy 的東西，跑過正交測試才發現裡面躺著三個 facet。**錯不在畫的人。**

**附帶一個例外**：前置篇 §2 說過「不要用尺來理解 facet」（軟膠囊不比硬膠囊「大」），但 `Too Sweet / Balanced / Not Sweet Enough` **確實有序**。凡是程度評價、滿意度、嚴重程度都這樣，叫 **ordinal（有序）facet**——它可以排序（「比 Balanced 更甜的那些」問得出來），而且**中間值不是 catch-all**：`Balanced` 是真實評價，不是「沒填」，兩者不分開就會把「沒人提到甜度」算成「大家覺得剛好」。

### 第二關：這一層真的有人要看嗎？

過了第一關——它確實是一棵樹——還有第二關。

Sweetness 是**一組平的值**，不需要階層。劑型可能需要：

```text
Dosage Form          Dosage Form
├── Gummy       →    ├── Solid
├── Capsule          │   ├── Tablet / Capsule / Gummy
├── Tablet           └── Liquid
└── Powder               └── Drop / Syrup / Drink
```
...
**② 上層那個名字會被單獨查詢。** 有人會問「固體劑型佔多少」嗎？會 → 需要階層，因為那個數字必須是底下所有子項的總和。不會 → **平的清單就夠了**。

第二條常被忽略，但它才是決定性的。前置篇 §4 講的 **lens（鏡頭）**——報表下拉選單裡「同一個維度看粗看細」——**只有階層撐得住**，因為粗的層級必須剛好等於細項的總和。**沒有人要看粗的，就不需要階層。**

劑型剛好示範了這件事：**評論分析**在乎的是具體型態，沒人問「固體劑型的評論佔多少」——平的清單就夠；**產品分類**的市場報表要按大類看，那就需要階層。

> **同一條軸，在不同場景可以有不同的形狀。** 這不是不一致，是因為「要不要階層」取決於你會怎麼查，而不是取決於這條軸本身。

**在檔案裡，過了第二關就是一個欄位的差別**：`hierarchical: false` 還是 `true`（見下一節）。沒過之前它只是一組合法值，**還不能叫 taxonomy**。
...
第二個理由才是維護。**三份東西的變更節奏差幾個數量級**——實例每天幾萬筆、facet 定義幾個月一次、值集合偶爾但要版控且由領域專家把關。混在一份裡它們會**互相綁架**：你想改一條軸的合法值，卻得動到幾萬筆實例資料所在的檔案；你想重跑標註，卻可能覆蓋掉領域專家的定義。
...
`catch_all` 與「沒提到」是兩個不同的值——**判準見第 3 節那段 ordinal 的說明**。檔案裡沒有 `catch_all`，或它跟「沒提到」共用一個值，都是問題。

> **為什麼這個 key 叫 `value_set` 而不是 `taxonomy`。** 照[分類體系術語](./classification-terminology.md)第 3 節，**taxonomy 是一棵樹**——`sweetness_v1` 是平的一組值，它還不是 taxonomy。兩者共用同一種檔案，用 `hierarchical` 區分：**`hierarchical: true` 的那些才是 taxonomy**（那時值改寫成 `nodes:`，每個節點帶 `broader:`），也就是第 3 節第二關說的「升級」。用同一個 key 稱呼兩者，會讓第 3 節第二關那條判準沒有東西可以落下來。

---

## 5. `too_sweet` 是什麼？——tag 的代價

回到第 2 節那個問題。同樣三個值，兩種記法：

```yaml
tags:   [too_sweet, effective, gummy]
facets: {sweetness: too_sweet, effectiveness: effective, dosage_form: gummy}
```
...
有一種講法是「人有背景知識可以猜得出來，**AI 更容易猜錯**」。**五次實測裡沒有出現這種情形**（見第 6 節，含限制）——Claude 面對這類模糊結構判斷得相當好，包括刻意設計的陷阱題。精確的說法是：

> **AI 猜得出來，但它每次猜的可能不一樣。**

你今天問，`too_sweet` 被歸到 `taste`；下週重問，被歸到 `sweetness`；同事那次，被歸到 `flavor_profile`。**每一次都合理，合起來無法對帳。**

所以 tag 的代價是**「每次都要重猜一次，而重猜的結果不保證一致」**。歸屬寫在檔案裡，就沒有人需要猜。

---

## 6. 實測：五次都沒搞混，但有三個失效點

五個情境，用「一般人真的會打的那句話」，單輪、不預告測試目的，題材是**保健食品分類檔**。

> **這五次證據有多強**（完整限制見文末）：**每個情境只跑一次**、**只測了一個模型**（Claude Opus 5）、**檔案都很短**、環境是 subagent。**結論只在「這五次裡」成立，不足以當成通則。**
...
**五題在「有沒有把正交的面壓成一棵樹」這一點上全對**——實測 2 自己講出了組合爆炸的後果，實測 4 自己從檔案裡找到「既有的 `forms` 是平行清單」當證據。

**但這裡評的只有這一件事，不是產出品質。** 照附錄 A 的清單看，實測 1 其實有問題：它只給 `OTHER_UNCLASSIFIED`，沒把 `other` 與 `unknown` 分開。**沒搞混概念，不等於檔案可以直接用。**
...
**這就是第 5 節那個結論的來源**：它未必猜錯，但不保證每次一樣。
...
| 區塊 | 回答 | 例 |
|---|---|---|
| **這份檔案是什麼** | 三份裡的哪一份、另外兩份在哪 | 「這是 facet 定義，實例在 `reviews/`，值集合在 `schema/value-sets/`，三者不得混寫」 |
| **哪些關係不能互推** | 每個關係欄位各自的意思 | 「`facet:` 是屬於哪條軸，`broader:` 是軸內部的直接上位——**縮排在本檔不帶語意**」 |
| **哪些東西不是階層** | 容易被當成上層的那些 | 「`groups` 只是導覽用，群組名稱不得出現在實例資料裡」 |
| **新增規則** | 要加東西時先問什麼 | 「先確定它屬於哪條既有的軸；不屬於任何一條，那它是新的軸」 |
| **已定案的約定** | 這一輪拍板的細節 | id 風格、label 不得當識別碼、`catch_all` 與「沒提到」不得合併 |

**它有效不是因為文字漂亮，是因為每一行都對應一個曾經被搞混的東西。** 所以別抄範本——**你的契約應該長什麼樣，取決於你這一輪被什麼絆倒過。**
...
**① 先問第 2 節那三個問題。** 不是要答得完美，是要**知道哪些你答不出來**。答不出來的先放 tag，但要明記「這是暫存區」（**本質上就開放的軸除外**，見第 2 節）。
...
- **實例 / facet 定義 / 值集合定義，三份分開**，各自有自己的變更節奏；值集合要有 id 和版本號，用 `values_ref` 串
...
**最後一條，也是最容易跳過的**：**驗證要靠程式，不要靠讀。** 模型的說明文字不是驗證。在**它產完之後、你收下之前**跑一次檢查（附錄 A），不要仰賴自己有沒有讀完說明。
...
**本文實測的限制**（第 6 節開頭有摘要，這裡是完整版。五次單輪對話，Claude Opus 5，獨立 session，2026-08-18）：
...
| 2.8 | 2026-08-21 | Dustin | `state` 更名為 `lifecycle`（生命週期）：repo 裡 compute-state-context.md 已用 state 指「呼叫之間要存活下來的資訊」，同字異義；lifecycle 與 stillflow 的文檔生命週期、eidos 的 issue lifecycle 同義，是既有用法的延伸。另：「身分」只保留識別那一義（與 eidos 一致），扮演那一義改敘述句；tag 兩小節合一並刪去 folksonomy 取捨表（那是選型者的東西）；instance 範例補上 `-` 使其只在語意上錯，並點出 `selected_*` 是最好認的指紋；第 4 節兜底值的解釋改為指回第 3 節，不再重述 |
| 2.7 | 2026-08-21 | Dustin | 原第 3、4 節合併為「看起來像樹的東西，要過兩關」：兩節底層是同一件事（不要多做一層），差別只在判準——第一關正交測試（它其實是幾條軸嗎）、第二關有沒有人要查（這一層需要嗎）。後續節次順推，內部交叉引用同步。另壓縮第 4 節變更節奏表、第 5 節 YAML 對照與結尾、第 6 節實測敘述。662 → 606 行 |
| 2.6 | 2026-08-21 | Dustin | 減量 738 → 660 行，刪的都是說了兩次的東西：契約範本由完整可抄的 YAML 改為五個區塊的說明（與第 5 節同一個理由，這篇給特徵不給成品）；第 9、10 節兩份總結合為一節；tag 的辨識留在第 2 節、代價留在第 6 節，不再各貼一次同樣的 YAML；第 3 節 mermaid 與身分對照表擇一留表；第 4 節場景對照表、第 1 節兩份範例、文末實測敘述各自壓縮 |
| 2.5 | 2026-08-21 | Dustin | 通讀後修九處：第 7 節「其他兜底值」的交叉引用由第 10 節更正為附錄 A（該守則不在第 10 節，issue #12 的 C2 原文亦誤植）；第 0 節開頭語氣改齊為「收下之前」；第 10 節「關於設計」改為「關於判斷」；第 2 節 tag 小節的「前面四個」依三個問題重寫；第 9 節流程圖與第一步改為問三個問題，不再並排五個名字 |
