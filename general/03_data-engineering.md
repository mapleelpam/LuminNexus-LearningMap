...
audience:
  - all
summary: |
  資料格式（JSON/CSV/YAML）、資料庫基礎、ETL 流程與資料充實技術。
---
...
> **⚠️ YAML 的縮排不等於階層語意。** 同一份分類體系，可以寫成一層層縮排的巢狀，
> 也可以寫成平的一列紀錄、每筆用一個欄位記自己的上層——**兩份檔案長得完全不同，
> 表達的卻是同一件事**。要用哪一種、以及各自有什麼保證是免費的、什麼要自己驗，
> 是在動筆之前就要決定的，見 [semantic-model-before-yaml.md](./semantic-model-before-yaml.md) 第 1 節。
...
| 1.2 | 2026-08-20 | 依 issue #12 修正 §1.5：寫法 A 範例改為合法且三層的 YAML；取捨表補「指到不存在的父節點」「兄弟順序」兩列，多父節點改列為警訊（該拆成兩條軸）；補 B 需要驗證機制（SKOS／qSKOS）的前提；解釋物化路徑、受控詞彙、繞圈 | Dustin |
| 1.3 | 2026-08-20 | 依 issue #12 移出 §1.5：該節在回答「同一格式內怎麼設計」，與第 1 章「有哪些格式、何時用哪個」不同層次，且深度超出本篇「概念理解」定位（00_outline 從未收錄此節）。內容改寫後移入 semantic-model-before-yaml.md 第 1 節，本篇留指路 | Dustin |
