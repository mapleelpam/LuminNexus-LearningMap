...
**而且這條界線會隨模型變強而右移**：今天需要澄清一輪的專案，明年可能一句話就夠。unknown 不是專案的固有屬性，是相對於**當期模型能力**的量——所以「這種規模要不要澄清」沒有永久答案，要定期重估。
...
| 1.1 | 2026-08-14 | maple | §1 補一句：unknown 低／高的界線會隨模型變強而右移——unknown 是相對於當期模型能力的量，不是專案的固有屬性 |
