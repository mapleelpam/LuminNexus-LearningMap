...
summary: |
  ...
  環境、市場的兩種切法與通路角色、voice 與滲透率，以及看數字之前該先問的五個問題。
...
翻成中文就是：「專注複方 共 750 毫克」，底下列了四種成分——**茶胺酸、假馬齒莧萃取、紅景天、銀杏**。（後面那個 `†` 是「這個成分沒有官方每日參考值」的標記，下一小節會講。）
...
>
> 分辨方法：**技術類型是一份「有哪幾種做法」的固定清單**，誰用了同一種做法就填同一個值；**商標則是某一家公司註冊的名字**，別人不能用。**做法回答「這是哪一類技術」，商標回答「這是誰家的」——後者才是 BT。**
>
> **怎麼分辨 BP**：它回答的是「**這個原料是從哪裡來的**」——特定海域、特定產區、特定廠的特定製程，而且那個來源被註冊成名字。
>
...
| 詞 | 白話 |
|---|---|
| **channel type（通路角色）** | 這個**賣家**在商業上扮演什麼角色。三種：電商平台、零售商、品牌官網 |
...
**資料從哪裡抓到的，不等於這個賣家的商業角色。** 從 Amazon 抓到的不見得是 Amazon 自營，從品牌官網抓到的也不保證是品牌直營——有些品牌把官網外包給經銷商經營。這兩件事是兩條互相推導不出來的軸，也就是 [classification-terminology.md](./classification-terminology.md) 第 2 節說的兩個 facet。
...
| **所有含南非醉茄**的產品 | 中等 | 「想用南非醉茄的品牌，有多少選了這個牌子的原料」 |
| **壓力情緒市場**的產品 | 較高 | 「在它主打的市場裡鋪得多廣」 |
...
> ⚠️ **「從哪裡抓到的」不等於「它在哪裡賣」**，也不等於這個賣家的商業角色——那是兩條互相推導不出來的軸（見第 12 節第 ④ 問）。
...
> 「**關節健康**這個依消費者需求切出來的**市場**，裡面**綜合維他命佔的比重偏高**——那類產品動輒含三、四十種成分，會把每個成分的普及率都灌水。**先把綜合維他命排除掉**，才看得出 **KSM-66 這個南非醉茄品牌原料，在有資格含它的產品裡實際鋪到多廣**。另外**從品牌官網抓到的那批產品評論數都是 0**，那是因為官網根本沒有評論功能，**不是這些產品沒人買**。」
...
| 1.0 | 2026-08-13 | leana | 定案：外部法規與認證來源逐條複驗；「六個數字陷阱」改寫為「看數字前先問的五個問題」（欄位名降為舉例，留下帶得走的問句）；系統實作細節（欄位路徑、ID 前綴、公式原文）移出本文。依 issue #5 盤點結果補上缺漏：第 1 節新增「標籤上的數字怎麼讀」（淨含量、每罐份數、%DV 與 RDA/RDI/AI/UL、三種單位系統、元素態、萃取比）與 proprietary blend、過敏原聲明、建議用法；第 2 節補 bioavailability；新增第 7 節「通路與商業結構」 |
