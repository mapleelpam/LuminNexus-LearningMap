...
**補集那條特別值得停下來想**。「非素食的產品」是哪個範圍裡的非素食——全站？某個品類裡？某個品牌的產品線裡？換一個全集，同一句話指的是完全不同的一批東西。
...
> A 底下有 B 和 C，而 **D 一部分在 B、一部分在 C**。

樹畫不出這個形狀。你只能把 D 塞進 B 或 C 其中一邊（丟掉一半資訊），或是把 D 複製兩份（從此兩份各自演化，總數也開始重複計算）。集合則直接寫得出來：D＝B 的一部分 ∪ C 的一部分。

**看到「這個東西該放哪邊？」這種爭論冒出來，通常不是分類沒想清楚，是形狀本身塞不進樹。**
