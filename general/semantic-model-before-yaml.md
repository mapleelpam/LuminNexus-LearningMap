...
**在探索階段，這是合理的**——你還不知道有幾條軸，先把觀察到的東西記下來。問題出在它留太久：**一旦有人開始拿 tag 做統計，那個「還沒想清楚」就變成了地基**（代價有多具體，見第 6 節）。
...
拿劑型當例子就很清楚：**評論分析**在乎的是具體型態，沒人會問「固體劑型的評論佔多少」——平的清單就夠；**產品分類**的市場報表要按大類看，那就需要階層。
...
**這裡評的只有這一件事，不是產出品質。** 照本文附錄 A 的檢查清單看，實測 1 其實有問題——它只給了 `OTHER_UNCLASSIFIED`，沒有把 `other` 與 `unknown` 分開（見下面的失效點二）。**沒搞混概念，不等於交出來的檔案可以直接用。**
...
**本文實測的限制**（第 7 節開頭有摘要，這裡是完整版。五次單輪對話，Claude Opus 5，獨立 session，2026-08-18）：
